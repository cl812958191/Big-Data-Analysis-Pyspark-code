...
def get_vector(x, NR, PR):
    return (np.max([0, (NR * ((1 + x[0]) / 2) * (1 - x[0] * (np.dot(w_current, x[1]))) + PR * ((1 - x[0]) / 2) * (
                1 - x[0] * (np.dot(w_current, x[1]))))]),
            np.zeros(Dimention) if (1 - x[0] * (np.dot(w_current, x[1]))) < 0 else (
                        (x[1] / 2) * (PR * (1 - x[0]) - NR * (1 + x[0]))))


if __name__ == "__main__":

    sc = SparkContext(appName="SVM_WL")
    # load training set
    start_time = time()
    ...
    print(
        'load data duration(s):%.4f, train model duration(s):%.4f, test model duration(s):%.4f total duration(s): %.4f'
